...
- штатного администратора можно создать при запуске через переменные окружения `APP_ADMIN_USERNAME`, `APP_ADMIN_PASSWORD`;
- имя администратора можно задать через `APP_ADMIN_FULL_NAME`;
- если пользователь с таким логином уже существует, он должен иметь роль администратора.
